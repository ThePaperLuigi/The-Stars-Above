...
{
    public class WolvesbaneAwakened : ModItem
	{
		...
		int wolvesbaneComboTimer;
		public override void HoldItem(Player player)
        {
            player.AddBuff(BuffID.Wrath, 2);

            wolvesbaneComboTimer--;
            ...
            {
                if (player.GetModPlayer<WeaponPlayer>().wolvesbaneGauge >= 100)
                {
                    ...
                    wolvesbaneComboTimer = 120;
                    player.SetDummyItemTime(20);
                    player.GetModPlayer<WeaponPlayer>().wolvesbaneGauge = 0;
                    ...
                    Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, Vector2.Zero, ProjectileType<WolvesbaneAwakenedSword>(), 0, 0, player.whoAmI, 0, 0, player.direction);
                    Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, Velocity, ProjectileType<WolvesbaneAwakenedShockwave>(), player.GetWeaponDamage(Item)*3, 0, player.whoAmI, 0, 0, player.direction);
			...
			if(wolvesbaneComboTimer == 100)
			{
                ...
                Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, Vector2.Zero, ProjectileType<WolvesbaneAwakenedSword>(), 0, 0, player.whoAmI, 0, 1, player.direction);
                Projectile.NewProjectile(player.GetSource_FromThis(), player.Center, Velocity, ProjectileType<WolvesbaneAwakenedShockwave>(), player.GetWeaponDamage(Item) * 3, 0, player.whoAmI, 0, 0, player.direction);
            ...
            if (wolvesbaneComboTimer < 100 && wolvesbaneComboTimer > 70)
            {
                for (int i = 0; i < 5; i++)
                {
                    // Charging dust
            ...
            if (wolvesbaneComboTimer == 70)
            {
                player.SetDummyItemTime(50);
                Vector2 direction = Vector2.Normalize(Main.MouseWorld - player.Center);
                Vector2 velocity = direction * (18);
                ...
                if (Collision.CanHit(newPos, 0, 0, newPos + muzzleOffset, 0, 0))
                {
                    newPos += muzzleOffset;
                }
                Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), newPos.X, newPos.Y, velocity.X, velocity.Y, ProjectileType<WolvesbaneGunEmpowered>(), 0, 0, player.whoAmI);

                Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), newPos.X, newPos.Y, velocity.X, velocity.Y, ProjectileType<WolvesbaneAwakenedBlastRound>(), player.GetWeaponDamage(Item) * 2, 0, player.whoAmI, 1f);
                ...
                for (int d = 0; d < 30; d++)//Visual effects
                {
                    ...
                    int dustIndex = Dust.NewDust(newPos, 0, 0, DustID.GemTopaz, perturbedSpeed.X, perturbedSpeed.Y, 150, default, 1.8f);
                ...
                for (int d = 0; d < 30; d++)//Visual effects
                {
                    ...
                    perturbedSpeed = perturbedSpeed * scale;
                    int dustIndex = Dust.NewDust(newPos, 0, 0, DustID.GemTopaz, perturbedSpeed.X, perturbedSpeed.Y, 150, default, 1f);
                    Main.dust[dustIndex].noGravity = true;
                ...
                newPos += Vector2.Normalize(new Vector2(velocity.X, velocity.Y)) * 20f;
                int dustAmount = 40;
                for (int i = 0; (float)i < dustAmount; i++)
                {
                    ...
                    int dust = Dust.NewDust(newPos, 0, 0, DustID.GemTopaz);
                    Main.dust[dust].scale = 2f;
                    Main.dust[dust].noGravity = true;
                    Main.dust[dust].position = newPos + spinningpoint5;
                    Main.dust[dust].velocity = velocity * 0f + spinningpoint5.SafeNormalize(Vector2.UnitY) * 3f;
                }
                newPos += Vector2.Normalize(new Vector2(velocity.X, velocity.Y)) * 10f;
                ...
                newPos -= Vector2.Normalize(new Vector2(velocity.X, velocity.Y)) * 30f;
                ...
                for (int d = 0; d < 20; d++)
                {
                    Dust.NewDust(player.Center, 0, 0, DustID.GemTopaz, Main.rand.NextFloat(-15, 15), Main.rand.NextFloat(-5, 5), 150, default, 0.7f);
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
		{
			if (player.altFunctionUse == 2)
			{
				Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), position.X, position.Y, velocity.X, velocity.Y, ProjectileType<WolvesbaneAwakenedStab>(), damage * 2, knockback, player.whoAmI, 1f);
                ...
                player.AddBuff(BuffType<Invincibility>(), 40);
                ...
                for (int d = 0; d < 30; d++)//Visual effects
                {
                    ...
                    float scale = -0.1f + Main.rand.NextFloat() * 0.3f;
                    perturbedSpeed = perturbedSpeed * scale;
                    int dustIndex = Dust.NewDust(position, 0, 0, DustID.GemTopaz, perturbedSpeed.X, perturbedSpeed.Y, 150, default, 1.8f);
                    Main.dust[dustIndex].noGravity = true;
                ...
                for (int d = 0; d < 30; d++)//Visual effects
                {
                    ...
                    float scale = 0.3f + Main.rand.NextFloat() * 0.3f;
                    perturbedSpeed = perturbedSpeed * scale;
                    int dustIndex = Dust.NewDust(position, 0, 0, DustID.GemTopaz, perturbedSpeed.X, perturbedSpeed.Y, 150, default, 1f);
                    Main.dust[dustIndex].noGravity = true;
			...
			else
            {
                ...
                switch (attackType)
                {
                    case 0: //Swing downwards
                        Projectile.NewProjectile(source, player.Center, Vector2.Zero, ProjectileType<WolvesbaneAwakenedSword>(), damage, knockback, player.whoAmI, 0, 0, player.direction);
                        attackType++;
                        return false;
                    case 1: //Swing upwards
                        Projectile.NewProjectile(source, player.Center, Vector2.Zero, ProjectileType<WolvesbaneAwakenedSword>(), damage, knockback, player.whoAmI, 0, 1, player.direction);
                        attackType++;

                        return false;
                    case 2: //Spin
                        Projectile.NewProjectile(source, player.Center, Vector2.Zero, ProjectileType<WolvesbaneAwakenedSwordSpin>(), damage, knockback, player.whoAmI, 0, 0, player.direction);
                        attackType = 0;
                        return false;
                }
