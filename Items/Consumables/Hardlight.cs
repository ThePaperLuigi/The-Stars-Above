...
namespace StarsAbove.Items.Consumables
{
    ...
    public class Hardlight : ModItem
	{
		...
		public override bool OnPickup(Player player)
        {
            ...
            int hardlightAcquired = Main.rand.Next(15, 45);
			Rectangle textPos = new Rectangle((int)player.position.X, (int)player.position.Y - 20, player.width, player.height);
			CombatText.NewText(textPos, new Color(222, 222, 222, 240), LangHelper.GetTextValue("Common.HardlightAcquired", hardlightAcquired), false, false);
			player.GetModPlayer<StarsAbovePlayer>().hardlight += hardlightAcquired;
			return false;
		}
		...
		{
			Lighting.AddLight(new Vector2(Item.Center.X, Item.Center.Y), 81 * 0.001f, 194 * 0.001f, 58 * 0.001f);//Code borrowed from the Spirit Mod! I'll make my own version someday!!
			for (int i = 0; i < 1; i++)
			{
				int num7 = 16;
				float num9 = 6f;
				...
				for (int index2 = 0; index2 < num7; ++index2)
				{
					...
					Main.spriteBatch.Draw((Texture2D)Terraria.GameContent.TextureAssets.Item[Item.type], position2, new Microsoft.Xna.Framework.Rectangle?(r), color3, rotation, vector2_3, Item.scale * 1.05f, spriteEffects, 0.0f);
				}
			}
			return true;
		}
