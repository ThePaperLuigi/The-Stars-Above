    ...
    public class StarsAboveSubworldNPC : GlobalNPC
    {
        ...
        public override void EditSpawnPool(IDictionary<int, float> pool, NPCSpawnInfo spawnInfo)
        {
            if (spawnInfo.Player.InModBiome<SeaOfStarsBiome>())
            {
                ...
                {
                    pool.Add(NPCType<NPCs.TownNPCs.Yojimbo>(), 0.1f);
            ...
            if (spawnInfo.Player.InModBiome<LyraBiome>()) // Anomaly planet stuff
            {
                ...
                pool.Add(NPCType<NPCs.PrismLoot>(), 0.3f);
            ...
            if (spawnInfo.Player.InModBiome<CorvusBiome>())
            {
                pool.Clear();
                pool.Add(NPCType<NPCs.PrismLoot>(), 0.05f);
                pool.Add(NPCType<NPCs.OffworldNPCs.AmethystHeadpiercer>(), 1f);
                pool.Add(NPCID.BlackSlime, 1f);
                pool.Add(NPCType<NPCs.OffworldNPCs.AmethystSwordsinner>(), 1f);
