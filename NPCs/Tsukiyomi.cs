    ...
    public class Tsukiyomi : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (NPC.ai[3] > 0f)//This is death effect
            {
                ...
                if (NPC.ai[3] >= 480f)
                {
                    ...
                    if (!NPC.AnyNPCs(NPCType<NPCs.Tsukiyomi2>()))
                    {
                        ...
                        NPC.NewNPC(NPC.GetSource_FromAI(),(int)NPC.Center.X, (int)NPC.Center.Y, NPCType<NPCs.Tsukiyomi2>());
                    }
            ...
            if(introAnimation > 0)
            {
                ...
                if(modPlayer.seenTsukiyomiIntro && introAnimation > 150)
                {
                    introAnimation = 150;
                }
            }
                ...
                if(phase >= 3)
                {
                    for (int i = 0; i < Main.maxPlayers; i++)
                    {
                        ...
                        if (player.active && modPlayer.inTsukiyomiFightTimer > 0)
                        {
                            ...
                            player.AddBuff(BuffType<Buffs.SubworldModifiers.MoonTurmoil>(), 30);
            ...
            if (nextAttack == "Elechos" && castTime == 28 && elechosAttack)
            {
                elechosAttack = false;
                for (int d = 0; d < 3200; d += 80)
                {

                    Vector2 placement = new Vector2((P.Center.X) + d, P.position.Y);
                    int type = Mod.Find<ModProjectile>("ThreadsOfFate").Type;


                    if(Main.netMode != NetmodeID.MultiplayerClient){Projectile.NewProjectile(NPC.GetSource_FromAI(),placement.X - 1600, placement.Y - 500, 0, 13,type,40,0f,Main.myPlayer);}
                ...
                if (castTime >= castTimeMax)
                {
                    ...
                    if (nextAttack == "Elechos")
                    {
                        ...
                        for (int d = 0; d < 4000; d += 100)
                        {
                            ...
                            if(Main.netMode != NetmodeID.MultiplayerClient){Projectile.NewProjectile(NPC.GetSource_FromAI(),placement.X - 1950, placement.Y - 600, 0, 13,type,40,0f,Main.myPlayer);}
                    ...
                    if(nextAttack == "Cosmic Rapture")
                    {
                        ...
                        if(Main.netMode != NetmodeID.MultiplayerClient){Projectile.NewProjectile(NPC.GetSource_FromAI(),placement.X, placement.Y, 0, 0,Mod.Find<ModProjectile>("BulletWormhole").Type,0,0f,Main.myPlayer);}
                        if(Main.netMode != NetmodeID.MultiplayerClient){Projectile.NewProjectile(NPC.GetSource_FromAI(),placement.X, placement.Y, 0, 0,Mod.Find<ModProjectile>("Wormhole").Type,0,0f,Main.myPlayer);}
                    ...
                    if (nextAttack == "Void Rapture")
                    {
                        ...
                        for (int i = 0; i < Main.maxPlayers; i++)
                        {
                            ...
                            if (player.active && modPlayer.inTsukiyomiFightTimer > 0)
                            {
                                ...
                                //player.AddBuff(BuffType<UmbralRapture>(), 240);
                                //player.AddBuff(BuffType<DownForTheCount>(), 240);
                            }

                        }
                        //if(Main.netMode != NetmodeID.MultiplayerClient){Projectile.NewProjectile(NPC.GetSource_FromAI(),new Vector2(NPC.Center.X - 370, NPC.Center.Y), Vector2.Zero, Mod.Find<ModProjectile>("EridaniBurstC").Type, 0, 0);}
                    ...
                    if (nextAttack == "Once Above, Ever Below")
                    {
                        ...
                        for (int i = 0; i < Main.maxPlayers; i++)
                        {
                            ...
                            if (player.active && modPlayer.inTsukiyomiFightTimer > 0)
                            {
                               ...
                               player.AddBuff(BuffType<Buffs.Invincibility>(), 280);  //Make sure to replace "buffType" and "timeInFrames" with actual values
            ...
            if (blazingSkiesTimer >= 40)
            {
                ...
                type = Main.rand.Next(new int[] { ProjectileID.PhantasmalBolt, ProjectileID.BrainScramblerBolt});

                if(type == ProjectileID.BrainScramblerBolt)
                {
                    Speed = 4f;
                }
                if (alignment == 2)
            ...
            if (NPC.ai[1] >= 0)
            {
                if (fightStart == true)
                {
                    ...
                    Vector2 initialMoveTo = P.Center + new Vector2(-55f, -150);
                    ...
                    for (int i = 0; i < Main.maxPlayers; i++)
                    {
                        ...
                        {
                            player.velocity = Vector2.Normalize(NPC.Center - player.Center) * -12f;
                        }
                    }
                    if (!Main.dedServ)
                    {
            ...
            if (NPC.ai[1] >= 500)//DEBUG IT SHOULD BE 500
            {
                ...
                if (!isCasting)
                {
                    ...
                    if (phase != 3 && NPC.life > 100000)
                    {
                        ...
                        if (NPC.ai[2] == 1)
                        {
                            nextCast = "Anosios";// Planet attack
                        }
                        if (NPC.ai[2] == 2)
                        {
                            nextCast = "Gateway";//
                        ...
                        if (NPC.ai[2] == 7)
                        {
                            nextCast = "Aetherial Subduction";//Rain of light
                        }
                        if (NPC.ai[2] == 8)
                        {
                        ...
                        if (NPC.ai[2] == 21)
                        {
                            nextCast = "Once Above, Ever Below";
                        ...
                        if (nextCast == "Hello, World")
                        {
                            ...
                            castAnimation = 70;
                            //Main.PlaySound(SoundLoader.customSoundType, (int)npc.Center.X, (int)npc.Center.Y, mod.GetSoundSlot(SoundType.Custom, "Sounds/Custom/DespairInOurPresence"));
                        ...
                        {
                            castAnimation = 70;
                            //Main.PlaySound(SoundLoader.customSoundType, (int)npc.Center.X, (int)npc.Center.Y, mod.GetSoundSlot(SoundType.Custom, "Sounds/Custom/DepriveThemOfLife"));
                            castDelay = 0;
                            nextAttack = "Cosmic Rapture";
                            castTime = 0;
                            castTimeMax = 50;
                    ...
                    // Special attacks /////////////////////////////////////////////////////////////////////////////////////////////
                    if (phase == 2 || (NPC.life <= 100000 && phase == 1))
                    {
                        castAnimation = 70;
                        ...
                        castTimeMax = 100;
                        isCasting = true;
                        nextAttack = "Once Above, Ever Below";
                        NPC.netUpdate = true;
                        NPC.dontTakeDamage = true;
                        NPC.ai[2] = 0;
