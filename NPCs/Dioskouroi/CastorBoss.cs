...
namespace StarsAbove.NPCs.Dioskouroi
{
	...
	public class CastorBoss : ModNPC
	{
		...
		public override void AI()
        {
            ...
            bossPlayer.CastorBarActive = true;
        ...
        public override void FindFrame(int frameHeight)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++)
			{
				...
				if (other.active && (other.type == ModContent.ProjectileType<CastorClashProjectile>()
		...
		private void SpawnAnimation()
		{
			...
			for (int d = 0; d < 144; d++)
			{
				Dust.NewDust(NPC.Center, 0, 0, DustID.FireworkFountain_Red, 0f + Main.rand.Next(-35, 35), 0f + Main.rand.Next(-35, 35), 150, default(Color), 1.5f);
			}
			for (int d = 0; d < 126; d++)
			{
				Dust.NewDust(NPC.Center, 0, 0, DustID.PinkFairy, 0f + Main.rand.Next(-36, 36), 0f + Main.rand.Next(-36, 36), 150, default(Color), 1.5f);
			}
		...
		private void Casting()
		{
			...
			NPC.localAI[3]++;
			NPC.direction = (Main.player[NPC.target].Center.X < NPC.Center.X).ToDirectionInt();//Face the target.
			modPlayer.CastTimeAlt = (int)NPC.localAI[3];
			modPlayer.CastTimeMaxAlt = (int)NPC.ai[3];

			if(NPC.frame.Y != (int)Frame.Empty)
            {
				//Idle dust because they're made out of magic.
				Dust d = Main.dust[Dust.NewDust(new Vector2(NPC.Center.X, NPC.Center.Y + 34), 0, 2, DustID.FireworkFountain_Red, Main.rand.NextFloat(-0.2f, 0.2f), Main.rand.NextFloat(-0.5f, -4.5f), 20, default(Color), 0.1f)];
				d.fadeIn = 1f;
				d.noGravity = true;
			}
