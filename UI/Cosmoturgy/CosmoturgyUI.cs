...
namespace StarsAbove.UI.Cosmoturgy
{
    internal class CosmoturgyUI : UIState
	{
		...
		private Dictionary<int, int> memoryIDs = new Dictionary<int, int>
		{
			...
			{ItemType<PrimeCut>(), 6 },
			{ItemType<PhantomMask>(), 7 },
			{ItemType<NetheriteBar>(), 8 },
			{ItemType<PearlescentOrb>(), 9 },
			{ItemType<ArgentumShard>(), 10 },
			{ItemType<PowerMoon>(), 11 },
			{ItemType<YoumuHilt>(), 12 },
			{ItemType<Rageblade>(), 13 },
			{ItemType<ElectricGuitarPick>(), 14 },
			{ItemType<DekuNut>(), 15 },
			{ItemType<MatterManipulator>(), 16 },
			{ItemType<BottledChaos>(), 17 },
			{ItemType<Trumpet>(), 18 },
