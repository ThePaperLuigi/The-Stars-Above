...
namespace StarsAbove.UI.QTEBar
{
    internal class QTEBar : UIState
	{
		...
		public override void OnInitialize() {
			...
			barFrame = new UIImage(Request<Texture2D>("StarsAbove/UI/QTEBar/QTEBar"));
		...
		public override void Draw(SpriteBatch spriteBatch) {
			...
			if (!(Main.LocalPlayer.GetModPlayer<BossPlayer>().QTEActive == true))
				return;
		...
		protected override void DrawSelf(SpriteBatch spriteBatch) {
			...
			// Calculate quotient
			float quotient = (float)modPlayer.QTEProgress / 100; // Creating a quotient that represents the difference of your currentResource vs your maximumResource, resulting in a float of 0-1f.
		...
		public override void Update(GameTime gameTime) {
			...
			text.SetText(LangHelper.GetTextValue($"CombatText.WarriorOfLight.QTE", StarsAbove.novaKey));
