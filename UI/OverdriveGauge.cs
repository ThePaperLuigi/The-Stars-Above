...
{
    internal class OverdriveGauge : UIState
	{
		...
		public override void OnInitialize() {
			...
			TopGradient2 = new Color(246, 140, 97); // Finish

			TopMiddleGradient1 = new Color(205, 160, 160); // 
			TopMiddleGradient2 = new Color(245, 79, 12); // 

			BottomGradient1 = new Color(165, 108, 108); // 
			BottomGradient2 = new Color(180, 55, 4); // 
		...
		public override void Draw(SpriteBatch spriteBatch) {
			if (!(Main.LocalPlayer.GetModPlayer<WeaponPlayer>().KissOfDeathHeld))
				return;
		...
		protected override void DrawSelf(SpriteBatch spriteBatch) {
			...
			// Calculate quotient
			float quotient = (float)modPlayer.overdriveGauge / (float)100; // Creating a quotient that represents the difference of your currentResource vs your maximumResource, resulting in a float of 0-1f.
			...
			spriteBatch.Draw((Texture2D)Request<Texture2D>("StarsAbove/UI/OverdriveGauge"), barFrame.GetInnerDimensions().ToRectangle(), Color.White);
