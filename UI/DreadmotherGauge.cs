...
{
    internal class DreadmotherGauge : UIState
	{
		...
		public override void OnInitialize() {
			...
			TopGradient1 = new Color(215, 189, 255); // Start
			TopGradient2 = new Color(137, 69, 241); // Finish

			TopMiddleGradient1 = new Color(192, 160, 205); // 
			TopMiddleGradient2 = new Color(135, 35, 175); // 

			BottomGradient1 = new Color(143, 108, 165); // 
			BottomGradient2 = new Color(110, 28, 164); // 
		...
		public override void Draw(SpriteBatch spriteBatch) {
            if (Main.LocalPlayer.GetModPlayer<WeaponPlayer>().dreadmotherHeld && Main.LocalPlayer.GetModPlayer<StarsAbovePlayer>().MeleeAspect == 2)
			{
		...
		protected override void DrawSelf(SpriteBatch spriteBatch) {
			...
			// Calculate quotient
			float quotient = (float)modPlayer.dreadmotherGauge / (float)100; // Creating a quotient that represents the difference of your currentResource vs your maximumResource, resulting in a float of 0-1f.
			...
			spriteBatch.Draw((Texture2D)Request<Texture2D>("StarsAbove/UI/DreadmotherGauge"), barFrame.GetInnerDimensions().ToRectangle(), Color.White);
