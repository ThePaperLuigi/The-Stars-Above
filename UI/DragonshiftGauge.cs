...
{
    internal class DragonshiftGauge : UIState
	{
		...
		public override void OnInitialize() {
			...
			TopGradient2 = new Color(236, 196, 104); // Finish

			TopMiddleGradient1 = new Color(205, 160, 160); // 
			TopMiddleGradient2 = new Color(255, 183, 56); // 

			BottomGradient1 = new Color(165, 108, 108); // 
			BottomGradient2 = new Color(209, 162, 54); // 
		...
		public override void Draw(SpriteBatch spriteBatch) {
			if (!(Main.LocalPlayer.GetModPlayer<WeaponPlayer>().DragaliaFoundHeld == true))
				return;
		...
		protected override void DrawSelf(SpriteBatch spriteBatch) {
			...
			// Calculate quotient
			float quotient = (float)modPlayer.DragonshiftGauge / (float)100; // Creating a quotient that represents the difference of your currentResource vs your maximumResource, resulting in a float of 0-1f.
			...
			spriteBatch.Draw((Texture2D)Request<Texture2D>("StarsAbove/UI/DragonshiftGauge"), barFrame.GetInnerDimensions().ToRectangle(), Color.White);
