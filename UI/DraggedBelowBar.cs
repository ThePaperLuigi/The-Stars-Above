...
{
    internal class DraggedBelowBar : UIState
	{
		...
		public override void OnInitialize()
		{
			...
			TopGradient1 = new Color(232, 190, 250); // Start
			TopGradient2 = new Color(198, 71, 253); // Finish

			TopMiddleGradient1 = new Color(210, 184, 225); // 
			TopMiddleGradient2 = new Color(164, 56, 225); // 

			BottomGradient1 = new Color(187, 173, 198); // 
			BottomGradient2 = new Color(130, 45, 196); // 
		...
		{
			if (!(Main.LocalPlayer.GetModPlayer<WeaponPlayer>().DraggedBelowHeld))
				return;
		...
		protected override void DrawSelf(SpriteBatch spriteBatch)
		{
			...
			// Calculate quotient
			float quotient = (float)modPlayer.DraggedBelowCorruption / (float)100; // Creating a quotient that represents the difference of your currentResource vs your maximumResource, resulting in a float of 0-1f.
			...
			spriteBatch.Draw((Texture2D)Request<Texture2D>("StarsAbove/UI/DraggedBelowBar"), barFrame.GetInnerDimensions().ToRectangle(), Color.White);
