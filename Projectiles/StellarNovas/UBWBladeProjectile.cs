...
namespace StarsAbove.Projectiles.StellarNovas
{
	...
	public class UBWBladeProjectile : ModProjectile
	{
		...
		public override void AI() {
			if (firstSpawn)
			{
				randomSize = Main.rand.Next(5, 30);
				Projectile.timeLeft = (int)Projectile.ai[2];
				Projectile.frame = Main.rand.Next(0, 14);
				//Replace rotation with a random small variation for the "blades embedded in the ground" effect
				Projectile.rotation = MathHelper.ToRadians(Main.rand.Next(-15, 15)) + MathHelper.ToRadians(135);
		...
		private void NormalAI() {
			...
			//If owner does not have UBW active, kill projectile else timeleft = 10
        ...
        public override void OnKill(int timeLeft) {
			...
			Vector2 rotationVector = (Projectile.rotation - MathHelper.ToRadians(45f)).ToRotationVector2(); // rotation vector to use for dust velocity
			usePos += rotationVector * 16f;

			// Spawn some dusts upon javelin death
			for (int i = 0; i < 20; i++) {
				// Create a new dust
				Dust dust = Dust.NewDustDirect(usePos, 0, 0, DustID.GemTopaz);
		...
		public override bool TileCollideStyle(ref int width, ref int height, ref bool fallThrough, ref Vector2 hitboxCenterFrac) {
			...
			width = height = randomSize; // notice we set the width to the height, the height to 10. so both are 10
			return true;
		}
