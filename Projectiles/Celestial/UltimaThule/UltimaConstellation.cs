...
namespace StarsAbove.Projectiles.Celestial.UltimaThule
{
    public class UltimaConstellation : ModProjectile
    {
            ...
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                ...
                if (proj.type == ProjectileType<UltimaConstellation>() && proj.timeLeft > 0)
                {
                    for (int i3 = 0; i3 < connectTo; i3++)
                    {
                        ...
                        Dust d = Dust.NewDustPerfect(position2, 20, null, 240, default, 0.3f);
                        d.fadeIn = 0.4f;
                        d.noLight = true;
                        d.noGravity = true;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                int dustIndex = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 20, 0f, 0f, 100, default, 0.8f);
                Main.dust[dustIndex].noGravity = true;
                Main.dust[dustIndex].velocity *= 5f;
                dustIndex = Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height, 20, 0f, 0f, 100, default, 0.8f);
