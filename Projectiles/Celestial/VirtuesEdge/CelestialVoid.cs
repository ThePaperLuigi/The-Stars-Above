    ...
    public class CelestialVoid : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (projOwner.GetModPlayer<WeaponPlayer>().VirtueGauge >= 100)
            {
                ...
                for (int i = 0; i < 100; i++)
                {
                    int dustIndex = Dust.NewDust(Projectile.Center, 0, 0, DustID.UnusedWhiteBluePurple, Main.rand.NextFloat(-25, 25), Main.rand.NextFloat(-25, 25), 100, default, 1f);
                    Main.dust[dustIndex].noGravity = true;

                    dustIndex = Dust.NewDust(Projectile.Center, 0, 0, DustID.PurpleCrystalShard, Main.rand.NextFloat(-20, 20), Main.rand.NextFloat(-20, 20), 100, default, 2f);
            ...
            else
            {
                for (int d = 0; d < 30; d++)
                {
                    Dust.NewDust(Projectile.Center, 0, 0, DustID.FireworkFountain_Blue, 0f + Main.rand.Next(-5, 5), 0f + Main.rand.Next(-5, 5), 150, default, 1.5f);
                }
                for (int d = 0; d < 44; d++)
                {
                    Dust.NewDust(Projectile.Center, 0, 0, DustID.GemSapphire, 0f + Main.rand.Next(-5, 5), 0f + Main.rand.Next(-5, 5), 150, default, 1.5f);
                }
                for (int d = 0; d < 26; d++)
                {
                    Dust.NewDust(Projectile.Center, 0, 0, DustID.Ice, 0f + Main.rand.Next(-5, 5), 0f + Main.rand.Next(-16, 16), 150, default, 1.5f);
                }
                for (int d = 0; d < 30; d++)
                {
                    Dust.NewDust(Projectile.Center, 0, 0, DustID.Firework_Blue, 0f + Main.rand.Next(-7, 7), 0f + Main.rand.Next(-7, 7), 150, default, 1.5f);
                }
            }
