...
namespace StarsAbove.Projectiles.Celestial.BlackSilence
{
    public class WheelsIndustrySlamDamage : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            for (int d = 0; d < 30; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.Obsidian, 0f + Main.rand.Next(-10, 10), 0f + Main.rand.Next(-10, 10), 150, default, 1.5f);
            }
            for (int d = 0; d < 44; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.Obsidian, 0f + Main.rand.Next(-15, 15), 0f + Main.rand.Next(-15, 15), 150, default, 1.5f);
            }
            for (int d = 0; d < 26; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.Obsidian, 0f + Main.rand.Next(-16, 16), 0f + Main.rand.Next(-16, 16), 150, default, 1.5f);
            }
            for (int d = 0; d < 30; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.SilverFlame, 0f + Main.rand.Next(-13, 13), 0f + Main.rand.Next(-13, 13), 150, default, 1.5f);
            }
            for (int d = 0; d < 40; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.SilverFlame, 0f + Main.rand.Next(-13, 13), 0f + Main.rand.Next(-13, 13), 150, default, 1.5f);
            ...
            for (int d = 0; d < 50; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.SilverFlame, 0f + Main.rand.Next(-15, 15), 0f + Main.rand.Next(-15, 15), 150, default, 1.5f);
            }
            // Smoke Dust spawn
            for (int i = 0; i < 70; i++)
            {
