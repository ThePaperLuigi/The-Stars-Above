...
namespace StarsAbove.Projectiles.TwinStars
{
    ...
    public class TwinStarLaser1 : ModProjectile
	{
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			if (IsAtMaxCharge) {
				DrawLaser(Main.spriteBatch, (Texture2D)TextureAssets.Projectile[Projectile.type], Main.player[Projectile.owner].GetModPlayer<StarsAbovePlayer>().starPosition1,
		...
		public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox) {
			...
			return Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), player.GetModPlayer<StarsAbovePlayer>().starPosition1,
				player.GetModPlayer<StarsAbovePlayer>().starPosition1 + unit * Distance, 22, ref point);
		}
		...
		public override void AI() {
			Player player = Main.player[Projectile.owner];
			Projectile.position = player.GetModPlayer<StarsAbovePlayer>().starPosition1 + Projectile.velocity * MOVE_DISTANCE;
		...
		private void SpawnDusts(Player player)
		{
			...
			for (int i = 0; i < 2; ++i)
			{
				...
				dust.noGravity = true;
				dust.scale = 1.2f;
				dust = Dust.NewDustDirect(Main.player[Projectile.owner].GetModPlayer<StarsAbovePlayer>().starPosition1, 0, 0, 31,
			...
			float rotation = (float)Math.Atan2(Main.player[Projectile.owner].GetModPlayer<StarsAbovePlayer>().starPosition1.Y - (player.GetModPlayer<StarsAbovePlayer>().starTarget.Y), Main.player[Projectile.owner].GetModPlayer<StarsAbovePlayer>().starPosition1.X - (player.GetModPlayer<StarsAbovePlayer>().starTarget.X));//Aim towards mouse
		...
		private void SetLaserPosition(Player player)
		{
			...
			{
				var start = player.GetModPlayer<StarsAbovePlayer>().starPosition1 + Projectile.velocity * Distance;
				if (!Collision.CanHit(player.GetModPlayer<StarsAbovePlayer>().starPosition1, 1, 1, start, 1, 1)) {
					Distance -= 5f;
					break;
				}
			}
		...
		private void ChargeLaser(Player player)
		{
			...
			else
			{
				...
				Vector2 pos = player.GetModPlayer<StarsAbovePlayer>().starPosition1 + offset - new Vector2(10, 10);
		...
		private void UpdatePlayer(Player player)
		{
			...
			if (Projectile.owner == Main.myPlayer) {
				Vector2 diff = player.GetModPlayer<StarsAbovePlayer>().starTarget - player.GetModPlayer<StarsAbovePlayer>().starPosition1;
				diff.Normalize();
				Projectile.velocity = diff;
				Projectile.direction = player.GetModPlayer<StarsAbovePlayer>().starTarget.X > player.GetModPlayer<StarsAbovePlayer>().starPosition1.X ? 1 : -1;
