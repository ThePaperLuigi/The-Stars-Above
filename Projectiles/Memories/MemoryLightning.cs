...
namespace StarsAbove.Projectiles.Memories
{
    public class MemoryLightning : ModProjectile
    {
			...
			*/
            ...
            if (Projectile.velocity == Vector2.Zero)
            {
                ...
                if (Main.rand.Next(Projectile.extraUpdates) == 0 && (Projectile.velocity != Vector2.Zero || Main.rand.Next(Projectile.localAI[1] == 2f ? 2 : 6) == 0))
                {
                    ...
                    if (Main.rand.Next(5) == 0)
                    {
                        ...
                        int num802 = Dust.NewDust(Projectile.Center + value37 - Vector2.One * 4f, 8, 8, 31, 0f, 0f, 100, default, 1.5f);
            ...
            else
            {
                ...
                while (true)
                {
                    ...
                    num805 %= 100;
                    float f2 = num805 / 100f * ((float)Math.PI * 2f);
                    ...
                    if (vector62.X * (Projectile.extraUpdates + 1) * 2f * num803 + Projectile.localAI[0] > 40f)
                    {
                        flag27 = true;
                    }
                    if (vector62.X * (Projectile.extraUpdates + 1) * 2f * num803 + Projectile.localAI[0] < -40f)
                ...
                Projectile.localAI[0] += spinningpoint7.X * (Projectile.extraUpdates + 1) * 2f * num803;
                ...
                {
                    float num806 = Main.rand.Next(-3, 4) * ((float)Math.PI / 3f) / 3f;
                    Vector2 vector63 = Projectile.ai[0].ToRotationVector2().RotatedBy(num806) * Projectile.velocity.Length();
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X - vector63.X, Projectile.Center.Y - vector63.Y, vector63.X, vector63.Y, Projectile.type, Projectile.damage / 2, Projectile.knockBack, Projectile.owner, vector63.ToRotation() + 1000f, Projectile.ai[1]);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int num360 = 0; num360 < 2; num360++)
            {
                ...
                if (num360 == 0)
                {
                    ...
                    DelegateMethods.c_1 = new Color(25, 45, 250, 0) * 0.5f; //This is the color of the lightning!
                }
                else
                {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            {
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y - 600, 0, 7, Projectile.type, Projectile.damage / 4, Projectile.knockBack, Projectile.owner, MathHelper.ToRadians(90), 1);
