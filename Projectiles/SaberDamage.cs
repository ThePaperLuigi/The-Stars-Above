...
namespace StarsAbove.Projectiles
{
    public class SaberDamage : ModProjectile
	{
		...
		public override void AI() {
			...
			for (int d = 0; d < 30; d++)
			{
				Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, 132, 0f + Main.rand.Next(-10, 10), 0f + Main.rand.Next(-10, 10), 150, default(Color), 1.5f);
			}
			for (int d = 0; d < 44; d++)
			{
				Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustType<Shine>(), 0f + Main.rand.Next(-15, 15), 0f + Main.rand.Next(-15, 15), 150, default(Color), 1.5f);
			}
			for (int d = 0; d < 26; d++)
			{
				Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, 20, 0f + Main.rand.Next(-16, 16), 0f + Main.rand.Next(-16, 16), 150, default(Color), 1.5f);
			}
