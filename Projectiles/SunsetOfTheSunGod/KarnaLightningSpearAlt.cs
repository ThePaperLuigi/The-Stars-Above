...
namespace StarsAbove.Projectiles.SunsetOfTheSunGod
{
    public class KarnaLightningSpearAlt : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if(!spawnDust)
			{
				...
				for (int ir = 0; ir < 30; ir++)
				{
					...
					Dust da = Dust.NewDustPerfect(positionNew, DustID.LifeDrain, null, 240, default(Color), 1f);
					da.fadeIn = 0.3f;
					da.noLight = true;
					da.noGravity = true;
		...
		public override bool PreDraw(ref Color lightColor)
		{
			...
			ArmorShaderData data = GameShaders.Armor.GetSecondaryShader((byte)GameShaders.Armor.GetShaderIdFromItemId(ItemID.InfernalWispDye), Main.LocalPlayer);
			data.Apply(null);
