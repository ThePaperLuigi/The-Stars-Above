...
{
    public class SakuraSlashVolcano2 : ModProjectile
	{
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
		{
			...
			for (int d = 0; d < 8; d++)
			{
				Dust.NewDust(target.Center, 0, 0, DustID.Fireworks, Main.rand.NextFloat(-5, 5), Main.rand.NextFloat(-5, 5), 150, default(Color), 0.4f);
			...
			if (!target.active)//On kill
			{
				...
				{
					// Random upward vector.
					Vector2 vel = new Vector2(Main.rand.NextFloat(-5, 5), Main.rand.NextFloat(-1, -3));
					Projectile.NewProjectile(Projectile.GetSource_FromThis(), target.Center, vel, ProjectileType<CursedFlames>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0, 1);
				}
			}
