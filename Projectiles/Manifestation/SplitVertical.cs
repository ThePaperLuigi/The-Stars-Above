...
namespace StarsAbove.Projectiles.Manifestation
{
    ...
    public class SplitVertical : ModProjectile
	{
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
			...
			for (int d = 0; d < 30; d++)
			{
				Dust.NewDust(target.Center, 0, 0, DustID.FireworkFountain_Red, 0f + Main.rand.Next(-20, 20), 0f + Main.rand.Next(-5, 5), 150, default(Color), 1.5f);
			}
			for (int d = 0; d < 44; d++)
			{
				Dust.NewDust(target.Center, 0, 0, DustID.LifeDrain, 0f + Main.rand.Next(-15, 15), 0f + Main.rand.Next(-15, 15), 150, default(Color), 1.5f);
			}
			for (int d = 0; d < 26; d++)
			{
				Dust.NewDust(target.Center, 0, 0, DustID.Firework_Red, 0f + Main.rand.Next(-16, 16), 0f + Main.rand.Next(-16, 16), 150, default(Color), 1.5f);
			}
