...
namespace StarsAbove.Projectiles.GoldenKatana
{
    ...
    public class GoldenKatanaChargeAttack2 : ModProjectile
	{
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
			for (int d = 0; d < 12; d++)
			{
				Dust.NewDust(target.Center, 0, 0, DustID.PurpleCrystalShard, 0f + Main.rand.Next(-4, 4), 0f + Main.rand.Next(-12, 12), 150, default(Color), 0.8f);
			}
			for (int d = 0; d < 32; d++)
			{
				Dust.NewDust(target.Center, 0, 0, DustID.Clentaminator_Purple, 0f + Main.rand.Next(-6, 6), 0f + Main.rand.Next(-6, 6), 150, default(Color), 1.2f);
			}
