...
namespace StarsAbove.Projectiles.Takodachi
{
    ...
    public class TakonomiconLaser : ModProjectile
	{
		...
		private void SpawnDusts(Player player)
		{
			...
			for (int i = 0; i < 2; ++i) {
				...
				Dust dust = Main.dust[Dust.NewDust(dustPos, 0, 0, 223, dustVel.X, dustVel.Y)];
				dust.noGravity = true;
				dust.scale = 1.2f;
		...
		private void ChargeLaser(Player player)
		{
			...
			else
			{
				...
				for (int k = 0; k < chargeFact + 1; k++)
				{
					...
					Dust dust = Main.dust[Dust.NewDust(pos, 20, 20, 223, Projectile.velocity.X / 2f, Projectile.velocity.Y / 2f)];
