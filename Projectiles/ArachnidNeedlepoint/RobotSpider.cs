...
namespace StarsAbove.Projectiles.ArachnidNeedlepoint
{
    ...
    public class RobotSpider : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (Projectile.ai[0] > 40)
			{
				...
				for (int i = 0; i < Main.maxProjectiles; i++)
				{
					...
					if (i != Projectile.whoAmI && other.active && other.owner == Projectile.owner && 
						(other.type == ProjectileType<RobotSpider>() && other.frame == 3 || other.type == ProjectileType<RobotSpiderStationary>()))
					{
						for (int ir = 0; ir < 50; ir++)
						{
							Vector2 position = Vector2.Lerp(Projectile.Center, other.Center, (float)ir / 50);
							Dust d = Dust.NewDustPerfect(position, 20, null, 240, default(Color), 0.7f);
							d.fadeIn = 0.3f;
							d.noLight = true;
							d.noGravity = true;
			...
			if(foundTarget && distanceFromTarget < 15f && latchCooldown < 0)
            {
				isLatchedOn = true;
			...
			if(isLatchedOn)
            {
				target.AddBuff(BuffType<LatchedOn>(), 10);
				latchCooldown = 120;
				Projectile.ai[0]++;
		...
		private bool CheckActive(Player owner)
		{
			...
			{
				owner.ClearBuff(ModContent.BuffType<RobotSpiderBuff>());

				return false;
			}

			if (owner.HasBuff(ModContent.BuffType<RobotSpiderBuff>()))
			{
