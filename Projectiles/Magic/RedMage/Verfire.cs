...
namespace StarsAbove.Projectiles.Magic.RedMage
{
    public class Verfire : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int d = 0; d < 16; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.Firework_Red, 0f + Main.rand.Next(-2, 2), 0f + Main.rand.Next(-2, 2), 150, default, 1.5f);
            }
            for (int d = 0; d < 10; d++)
            {
            ...
            for (int d = 0; d < 10; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.LifeDrain, 0f + Main.rand.Next(-3, 3), 0f + Main.rand.Next(-3, 3), 150, default, 1.5f);
            }
            // Smoke Dust spawn
            for (int i = 0; i < 20; i++)
            {
