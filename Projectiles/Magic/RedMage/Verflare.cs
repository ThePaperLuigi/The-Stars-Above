...
{
    public class Verflare : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            // DisplayName.SetDefault("Verflare");     //The English name of the Projectile
        ...
        public override void AI()
        {
            ...
            Color newColor = Main.hslToRgb(0.12f, 1f, 0.50f) * Projectile.Opacity;
            float num4 = 6f;
            float num5 = 2f;
            if (num10 < 0.9f)
            {
                for (int i = 0; i < 3; i++)
                {
                    if (Main.rand.Next(2) == 0)
                    {
                        ...
                        Vector2 value = vector * (num3 * Projectile.scale);
            ...
            if (num10 < 0.9f)
            {
                for (int j = 0; j < 3; j++)
                {
                    if (Main.rand.Next(2) == 0)
                    {
                        ...
                        Vector2 value3 = vector2 * (num3 * Projectile.scale);
            ...
            if (num10 < 0.95f)
            {
                for (float num8 = 0f; num8 < 0.8f; num8 += 1f)
                {
                    ...
                    {
                        Vector2 value5 = Vector2.UnitX.RotatedBy(Main.rand.NextFloat() * ((float)Math.PI * 2f) + (float)Math.PI / 2f) * (num3 * Projectile.scale);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, 0, 0, ProjectileType<VerflareExplosion>(), Projectile.damage * 3, 0f, 0, 0);
