    ...
    public class OzmaAttack1 : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (projOwner.HasBuff(BuffType<AnnihilationState>()))
            {
                for (int i = 0; i < 22; i++)
                {
                    int dustIndex = Dust.NewDust(Projectile.Center, 0, 0, DustID.LifeDrain, 0f + Main.rand.Next(-6, 6), 0f + Main.rand.Next(-6, 6), 100, default, 1.1f);
                    //Main.dust[dustIndex].velocity *= 0.6f;
                }
                for (int i = 0; i < 22; i++)
                {
                    int dustIndex = Dust.NewDust(Projectile.Center, 0, 0, DustID.PlatinumCoin, 0f + Main.rand.Next(-6, 6), 0f + Main.rand.Next(-6, 6), 100, default, 1.1f);
                    //Main.dust[dustIndex].velocity *= 0.6f;
                }
            }
            else
