...
namespace StarsAbove.Projectiles.Magic.DraggedBelow
{
    public class DraggedBelowFloodExplosionCorrupted : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int d = 0; d < 50; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.GemRuby, 0f + Main.rand.Next(-10, 10), 0f + Main.rand.Next(-5, 5), 150, default, 1.5f);
            }
            for (int d = 0; d < 54; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.FireworkFountain_Red, 0f + Main.rand.Next(-25, 25), 0f + Main.rand.Next(-25, 25), 150, default, 1.5f);
            }
