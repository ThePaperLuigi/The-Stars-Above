...
namespace StarsAbove.Projectiles.Magic.DraggedBelow
{
    // The following laser shows a channeled ability, after charging up the laser will be fired
    // Using custom drawing, dust effects, and custom collision checks for tiles
    public class DraggedBelowLaser : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (IsAtMaxCharge)
            {
                ...
                ArmorShaderData data = GameShaders.Armor.GetSecondaryShader((byte)GameShaders.Armor.GetShaderIdFromItemId(ItemID.MirageDye), Main.LocalPlayer);
                data.Apply(null);
                DrawLaser(Main.spriteBatch, (Texture2D)TextureAssets.Projectile[Projectile.type], Main.player[Projectile.owner].MountedCenter,
                    Projectile.velocity, 10, Projectile.damage, -1.57f, 1f, 200f, Color.White, (int)MOVE_DISTANCE);
        ...
        private void SpawnDusts(Player player)
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                Dust dust = Main.dust[Dust.NewDust(dustPos, 0, 0, DustID.GemAmethyst, dustVel.X, dustVel.Y)];
                dust.noGravity = true;
                dust.scale = 1.2f;

            }
            float rotation = (float)Math.Atan2(Main.player[Projectile.owner].MountedCenter.Y - player.GetModPlayer<WeaponPlayer>().DraggedBelowTarget.Y, Main.player[Projectile.owner].MountedCenter.X - player.GetModPlayer<WeaponPlayer>().DraggedBelowTarget.X);//Aim towards mouse

            for (int d = 0; d < 5; d++)
            {
                float Speed2 = Main.rand.NextFloat(1, 40);  //projectile speed
                ...
                int dustIndex = Dust.NewDust(Main.player[Projectile.owner].MountedCenter - Vector2.One * 4f, 0, 0, DustID.Shadowflame, perturbedSpeed.X, perturbedSpeed.Y, 150, default, 1f);
            ...
            for (int d = 0; d < 12; d++)
            {
                float Speed2 = Main.rand.NextFloat(1, 40);  //projectile speed
                Vector2 perturbedSpeed = new Vector2((float)(Math.Cos(rotation) * Speed2 * -1), (float)(Math.Sin(rotation) * Speed2 * -1)).RotatedByRandom(MathHelper.ToRadians(7)); // 30 degree spread.
                int dustIndex = Dust.NewDust(Main.player[Projectile.owner].MountedCenter - Vector2.One * 4f, 0, 0, DustID.GemAmethyst, perturbedSpeed.X, perturbedSpeed.Y, 150, default, 1f);
                Main.dust[dustIndex].noGravity = true;
            }
            for (int d = 0; d < 2; d++)
            {
                float Speed2 = Main.rand.NextFloat(1, 40);  //projectile speed
                Vector2 perturbedSpeed = new Vector2((float)(Math.Cos(rotation) * Speed2 * -1), (float)(Math.Sin(rotation) * Speed2 * -1)).RotatedByRandom(MathHelper.ToRadians(7)); // 30 degree spread.
                int dustIndex = Dust.NewDust(Main.player[Projectile.owner].MountedCenter - Vector2.One * 4f, 0, 0, DustID.PinkFairy, perturbedSpeed.X, perturbedSpeed.Y, 150, default, 1f);
                Main.dust[dustIndex].noGravity = true;
            }
        }
        ...
        private void SetLaserPosition(Player player)
        {
            ...
            {
                var start = player.MountedCenter + Projectile.velocity * Distance;
                if (!Collision.CanHit(player.MountedCenter, 1, 1, start, 1, 1))
                {
                    Distance -= 5f;
                    break;
                }
        ...
        private void ChargeLaser(Player player)
        {
            ...
            if (!player.HasBuff(BuffType<DraggedBelowDrownActive>()))
            {
                Projectile.Kill();
            }
            else
        ...
        private void UpdatePlayer(Player player)
        {
            ...
            {
                Vector2 diff = player.GetModPlayer<WeaponPlayer>().DraggedBelowTarget - player.MountedCenter;
                diff.Normalize();
                Projectile.velocity = diff;
                Projectile.direction = player.GetModPlayer<WeaponPlayer>().DraggedBelowTarget.X > player.MountedCenter.X ? 1 : -1;
