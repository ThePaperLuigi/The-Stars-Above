...
{
    public class TwinStarShine2 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            double dist = 34; //Distance away from the player

            /*Position the player based on where the player is, the Sin/Cos of the angle times the /
            /distance for the desired distance away from the player minus the projectile's width   /
            /and height divided by two so the center of the projectile is at the right place.     */
            Projectile.position.X = player.GetModPlayer<WeaponPlayer>().starPosition2.X - (int)(Math.Cos(rad) * dist) - Projectile.width / 2;
            Projectile.position.Y = player.GetModPlayer<WeaponPlayer>().starPosition2.Y - (int)(Math.Sin(rad) * dist) - Projectile.height / 2;
