...
namespace StarsAbove.Projectiles.Melee.ClaimhSolais
{
    public class Erinys : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.frame != 13)
            {
                Projectile.position.Y = ownerMountedCenter.Y - Projectile.height / 2 - 180;
            }
            if (Projectile.frame == 1)
            {
                ...
                for (int d = 0; d < 6; d++)
                {
                    Dust.NewDust(new Vector2(Projectile.Center.X - 15, Projectile.Center.Y), 0, 0, DustType<Dusts.Star>(), 0f + Main.rand.Next(-10, 10), 0f + Main.rand.Next(-10, 10), 150, default, 1.5f);
                }

                for (int d = 0; d < 9; d++)
                {
                    Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y), 0, 0, 133, 0f + Main.rand.Next(-26, 26), 0f + Main.rand.Next(-26, 26), 150, default, 1.5f);
                }
