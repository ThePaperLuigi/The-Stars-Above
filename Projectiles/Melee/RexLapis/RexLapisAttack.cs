...
namespace StarsAbove.Projectiles.Melee.RexLapis
{
    public class RexLapisAttack : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int d = 0; d < 14; d++)
            {
                Dust.NewDust(target.Center, 0, 0, 222, Main.rand.NextFloat(-5, 5), Main.rand.NextFloat(-5, 5), 150, default, 0.3f);
            ...
            if (hit.Crit && target.HasBuff(BuffType<Petrified>()))
            {
                ...
                for (int d = 0; d < 14; d++)
                {
                    Dust.NewDust(target.Center, 0, 0, DustID.FireworkFountain_Yellow, Main.rand.NextFloat(-25, 25), Main.rand.NextFloat(-25, 25), 150, default, 1f);
