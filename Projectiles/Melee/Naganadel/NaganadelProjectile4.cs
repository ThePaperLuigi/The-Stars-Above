...
namespace StarsAbove.Projectiles.Melee.Naganadel
{
    public class NaganadelProjectile4 : ModProjectile
    {
        ...
        {
            Lighting.AddLight(Projectile.Center, new Vector3(0.99f, 0.4f, 0.99f));
            // Since we access the owner player instance so much, it's useful to create a helper local variable for this
            // Sadly, Projectile/ModProjectile does not have its own
            Player projOwner = Main.player[Projectile.owner];
            if (projOwner.HeldItem.ModItem is Items.Weapons.Melee.Naganadel)
            {
            ...
            if (projOwner.GetModPlayer<WeaponPlayer>().naganadelWeapon4Summoned == false)
            {
                if (Projectile.owner == Main.myPlayer)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position.X + Projectile.width / 2, Projectile.position.Y + Projectile.height / 2, Main.MouseWorld.ToRotation(), Main.MouseWorld.ToRotation(), ProjectileType<NaganadelProjectileFinal4>(), Projectile.damage * 10, 0f, Projectile.owner, 0f, 0f);
