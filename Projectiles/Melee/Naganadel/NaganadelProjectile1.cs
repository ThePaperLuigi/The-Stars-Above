...
{
    public class NaganadelProjectile1 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (!projOwner.frozen)
            {
                ...
                {
                    movementFactor = 30f;
                    /*vector32 = projectile.Center;
                    for (int i = 0; i < 50; i++)
                    {
                        Vector2 position = Vector2.Lerp(projOwner.Center, vector32, (float)i / 50);
            ...
            if (projOwner.GetModPlayer<WeaponPlayer>().naganadelWeapon1Summoned == false)
            {
                if (Projectile.owner == Main.myPlayer)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position.X + Projectile.width / 2, Projectile.position.Y + Projectile.height / 2, Main.MouseWorld.ToRotation(), Main.MouseWorld.ToRotation(), ProjectileType<NaganadelProjectileFinal1>(), Projectile.damage * 10, 0f, Projectile.owner, 0f, 0f);
