...
namespace StarsAbove.Projectiles.Summon.DragaliaFound
{
    public class DragonTornado : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (float num283 = (int)value104.Y; num283 > (int)value103.Y; num283 -= num282)
            {
                num281 += num282;
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < dustAmount; i++)
            {
                ...
                spinningpoint5 += -Vector2.UnitY.RotatedBy(i * ((float)Math.PI * 2f / dustAmount)) * new Vector2(14f, 2f);
            ...
            {
                Vector2 spinningpoint5 = Vector2.UnitX * 0f;
                spinningpoint5 += -Vector2.UnitY.RotatedBy(i * ((float)Math.PI * 2f / dustAmount)) * new Vector2(140f, 6f);
            ...
            {
                Vector2 spinningpoint5 = Vector2.UnitX * 0f;
                spinningpoint5 += -Vector2.UnitY.RotatedBy(i * ((float)Math.PI * 2f / dustAmount)) * new Vector2(20f, 1f);
                spinningpoint5 = spinningpoint5.RotatedBy(Projectile.velocity.ToRotation() + MathHelper.ToRadians(90));
                int dust = Dust.NewDust(target.Center, 0, 0, DustID.GemEmerald);
                Main.dust[dust].scale = 2f;
                Main.dust[dust].noGravity = true;
