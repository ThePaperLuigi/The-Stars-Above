...
{
    public class Kifrosse4 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (player.ownedProjectileCounts[ProjectileType<Projectiles.Summon.Kifrosse.DancingFoxfire1>()] < 1)
            {
                int index = Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.position.X, player.position.Y, 0, 0, ProjectileType<Projectiles.Summon.Kifrosse.DancingFoxfire1>(), Projectile.damage, 4, player.whoAmI, 0f);
                Main.projectile[index].originalDamage = Projectile.damage;

            }
            if (player.ownedProjectileCounts[ProjectileType<Projectiles.Summon.Kifrosse.DancingFoxfire2>()] < 1)
            {
                int index = Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.position.X, player.position.Y, 0, 0, ProjectileType<Projectiles.Summon.Kifrosse.DancingFoxfire2>(), Projectile.damage, 4, player.whoAmI, 0f);
                Main.projectile[index].originalDamage = Projectile.damage;
            ...
            if (amaterasu > 480)
            {
                for (int d = 0; d < 6; d++)
                {
                    Dust.NewDust(player.position, Projectile.width, Projectile.height, 221, 0f + Main.rand.Next(-2, 2), 0f + Main.rand.Next(-2, 2), 150, default, 1.5f);
                }
