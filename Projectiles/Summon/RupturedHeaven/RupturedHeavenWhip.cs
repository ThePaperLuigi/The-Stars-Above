...
namespace StarsAbove.Projectiles.Summon.RupturedHeaven
{
    public class RupturedHeavenWhip : ModProjectile
    {
            ...
            if (hit.Crit)
            {
                for (int i = 0; i < Main.maxPlayers; i++)
                {
                    ...
                    if (p.active && p.Distance(Main.player[Projectile.owner].Center) < 1000)
                    {
                        ...
                        for (int d = 0; d < 15; d++)
                        {
                            Dust.NewDust(p.Center, 0, 0, DustID.MagicMirror, 0f + Main.rand.Next(-5, 5), 0f + Main.rand.Next(-5, 5), 150, default, 0.8f);
                        }
                    }
        ...
        public override void CutTiles()
        {
            Projectile.WhipPointsForCollision.Clear();
            FillWhipControlPoints(Projectile, Projectile.WhipPointsForCollision);
            Vector2 zero = new Vector2(Projectile.width * Projectile.scale / 2f, 0f);
