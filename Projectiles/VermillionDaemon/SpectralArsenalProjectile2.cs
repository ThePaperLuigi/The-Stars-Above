...
namespace StarsAbove.Projectiles.VermillionDaemon
{
    public class SpectralArsenalProjectile2 : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if(player.GetModPlayer<WeaponPlayer>().SpectralArsenal == 2)
            {
				Projectile.rotation = MathHelper.ToRadians(player.GetModPlayer<WeaponPlayer>().DaemonGlobalRotation) + MathHelper.ToRadians(45);
			}
			else
            {
				Projectile.rotation = MathHelper.ToRadians(player.GetModPlayer<WeaponPlayer>().DaemonGlobalRotation) + MathHelper.ToRadians(30);
