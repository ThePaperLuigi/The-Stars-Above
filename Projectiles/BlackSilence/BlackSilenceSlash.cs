		...
		public override void AI() {
			...
			if (Projectile.ai[0] == 1)
			{
				...
				for (int d = 0; d < 2; d++)
				{
					...
					int dustIndex = Dust.NewDust(Projectile.Center, 0, 0, DustID.SilverFlame, perturbedSpeed.X, perturbedSpeed.Y, 150, default(Color), 1f);
				...
				for (int d = 0; d < 3; d++)
				{
					...
					int dustIndex = Dust.NewDust(Projectile.Center, 0, 0, DustID.Obsidian, perturbedSpeed.X, perturbedSpeed.Y, 150, default(Color), 2f);
					Main.dust[dustIndex].noGravity = true;
				}
				/*for (int d = 0; d < 2; d++)
				{
