...
{
    public class EmblazonedAmethyst : ModProjectile
	{
		public override void SetStaticDefaults() {
			// DisplayName.SetDefault("Amethyst Star");     //The English name of the projectile
		...
		public override void AI()
        {
			...
			int dustIndex = Dust.NewDust(Projectile.Center, 0, 0, 223, Main.rand.NextFloat(-1, 1), Main.rand.NextFloat(-1, 1), 100, default(Color), 1f);
		...
		public override void Kill(int timeLeft)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				if (Main.rand.Next(10) == 1)
				{
					...
					int k = Item.NewItem(null, (int)Projectile.Center.X + Main.rand.Next(-50, 50), (int)Projectile.Center.Y + Main.rand.Next(-50, 50), 0, 0, Mod.Find<ModItem>("AmethystFragment").Type, 1, false);
					if (Main.netMode == 1)
					{
			...
			for (int i = 0; i < 70; i++)
			{
				int dustIndex = Dust.NewDust(Projectile.Center, 0, 0, 223, Main.rand.NextFloat(-25,25), Main.rand.NextFloat(-25, 25), 100, default(Color), 1f);
				Main.dust[dustIndex].noGravity = true;
				
				dustIndex = Dust.NewDust(Projectile.Center, 0, 0, 223, Main.rand.NextFloat(-20, 20), Main.rand.NextFloat(-20, 20), 100, default(Color), 2f);
