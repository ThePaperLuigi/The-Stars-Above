...
{
    public class ImmemorialSupernovaProjectile : ModProjectile
	{
		public override void SetStaticDefaults() {
			// DisplayName.SetDefault("Immemorial Supernova");     //The English name of the projectile
		...
		public override void OnKill(int timeLeft)
		{
			...
			Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, 0, 0, Mod.Find<ModProjectile>("EternalStarburst").Type, Projectile.damage*5, 0f, 0, 0);
			...
			for (int i = 0; i < 100; i++)
			{
				int dustIndex = Dust.NewDust(Projectile.Center, 0, 0, 91, Main.rand.NextFloat(-25, 25), Main.rand.NextFloat(-25, 25), 100, default(Color), 1f);
				Main.dust[dustIndex].noGravity = true;

				dustIndex = Dust.NewDust(Projectile.Center, 0, 0, 91, Main.rand.NextFloat(-20, 20), Main.rand.NextFloat(-20, 20), 100, default(Color), 2f);
