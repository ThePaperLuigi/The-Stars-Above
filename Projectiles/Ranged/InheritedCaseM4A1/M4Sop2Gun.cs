...
{
    public class M4Sop2Gun : InheritedCaseOrbitalsGun
    {
        public override string Texture => "StarsAbove/Projectiles/Ranged/InheritedCaseM4A1/M4Sop2Gun";
        ...
        public override int MuzzleDistance => 14;
        ...
        public override int ReActivateAfterIdleTimer => 120;
        ...
        public override void PostAI()
        {
            ...
            if (Projectile.ai[0] == 0 && Projectile.localAI[1] >= 1)
            {
                ...
                {
                    Projectile.NewProjectile(projOwner.GetSource_ItemUse(projOwner.HeldItem), position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y, ProjectileType<M4Sop2HowitzerHoming>(), projOwner.GetWeaponDamage(projOwner.HeldItem), 1, projOwner.whoAmI);
                    Projectile.localAI[0] = 0;

                }
                else
                {
                    Projectile.NewProjectile(projOwner.GetSource_ItemUse(projOwner.HeldItem), position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y, ProjectileType<M4Sop2Howitzer>(), projOwner.GetWeaponDamage(projOwner.HeldItem), 1, projOwner.whoAmI);
        ...
        public override void OnKill(int timeLeft)
        {
            for (int d = 0; d < 20; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.FireworkFountain_Red, 0f + Main.rand.Next(-4, 4), 0f + Main.rand.Next(-4, 4), 150, default(Color), 0.8f);
            }
