...
{
    public class RO635Gun : InheritedCaseOrbitalsGun
    {
        public override string Texture => "StarsAbove/Projectiles/Ranged/InheritedCaseM4A1/RO635Gun";
        ...
        public override void PostAI()
        {
            ...
            if (Projectile.ai[0] == 0 && Projectile.localAI[1] >= 1)
            {
                ...
                Projectile.NewProjectile(projOwner.GetSource_ItemUse(projOwner.HeldItem), position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y, ProjectileType<RO635Round>(), projOwner.GetWeaponDamage(projOwner.HeldItem) / 3, 1, projOwner.whoAmI);
        ...
        public override void OnKill(int timeLeft)
        {
            for (int d = 0; d < 20; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.FireworkFountain_Yellow, 0f + Main.rand.Next(-4, 4), 0f + Main.rand.Next(-4, 4), 150, default(Color), 0.8f);
            }
