...
{
    public class M16A1Gun : InheritedCaseOrbitalsGunSpecial
    {
        public override string Texture => "StarsAbove/Projectiles/Ranged/InheritedCaseM4A1/M16A1Gun";
        ...
        public override int ReActivateAfterIdleTimer => 10;
        ...
        public override void PostAI()
        {
            ...
            if (projOwner.ownedProjectileCounts[ProjectileType<M16A1Shield>()] < 1 && !projOwner.HasBuff(BuffType<M16A1ShieldCooldown>()))
            {//Equip animation.
                int index = Projectile.NewProjectile(projOwner.GetSource_ItemUse(projOwner.HeldItem), projOwner.position.X, projOwner.position.Y, 0, 0, ProjectileType<M16A1Shield>(), 0, 0, projOwner.whoAmI, 0f,0f,6f);
            }
            var modPlayer = projOwner.GetModPlayer<WeaponPlayer>();
            ...
            if (Projectile.ai[0] == 0 && Projectile.localAI[1] >= 1)
            {
                ...
                {
                    Vector2 perturbedSpeed = new Vector2(arrowVelocity.X, arrowVelocity.Y).RotatedBy(MathHelper.ToRadians(i));
                    float scale = 1f;
                    perturbedSpeed = perturbedSpeed * scale;
                    Projectile.NewProjectile(projOwner.GetSource_ItemUse(projOwner.HeldItem), position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y, ProjectileType<M16A1Round>(), projOwner.GetWeaponDamage(projOwner.HeldItem), 1, projOwner.whoAmI);
                }
            }
