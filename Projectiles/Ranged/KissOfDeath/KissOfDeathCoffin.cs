...
namespace StarsAbove.Projectiles.Ranged.KissOfDeath
{
    ...
    public class KissOfDeathCoffin : ModProjectile
    {
        ...
        public override void SetDefaults()
        {
            ...
            Projectile.width = 210; // The width of your projectile
            Projectile.height = 210; // The height of your projectile
            Projectile.friendly = true; // Deals damage to enemies
            Projectile.penetrate = -1; // Infinite pierce
            Projectile.DamageType = DamageClass.Ranged; // Deals melee damage
        ...
        public override void AI()
        {
            ...
            int movingHitCooldown = 120; // How often your flail hits when moving
            ...
            float meleeSpeedMultiplier = player.GetTotalAttackSpeed(DamageClass.Ranged);
                ...
                case AIState.Spinning:
                    {
                        ...
                        Projectile.Center = mountedCenter + offsetFromPlayer * 140f;
                ...
                case AIState.UnusedState: // Projectile.ai[0] == 3; This case is actually unused, but maybe a Terraria update will add it back in, or maybe it is useless, so I left it here.
                    {
                        ...
                        Projectile.tileCollide = false;//= StateTimer == 1f;
        ...
        public override bool? CanDamage()
        {
            ...
            if (CurrentAIState == AIState.Spinning && SpinningStateTimer <= 5f)
            {
                //return false;
            }
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            ...
            /*if (CurrentAIState == AIState.Spinning)
			{
				...
				float hitRadius = 155f; // The length of the semi-major radius of the ellipse (the long end)
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            if (target.HasBuff<SecurityLevel>())
            {
                int index = target.FindBuffIndex(BuffType<SecurityLevel>());
                if (index >= 0)
                    target.DelBuff(index);

                projOwner.GetModPlayer<WeaponPlayer>().overdriveGauge += 10;
                projOwner.GetModPlayer<WeaponPlayer>().gaugeChangeAlpha = 1f;
                if (projOwner.GetModPlayer<WeaponPlayer>().overdriveGauge > 100)
                {
                    projOwner.GetModPlayer<WeaponPlayer>().overdriveGauge = 100;
            ...
            for (int d = 0; d < 18; d++)
            {
                Dust.NewDust(target.Center, 0, 0, DustID.AmberBolt, Main.rand.NextFloat(-5, 5), Main.rand.NextFloat(-5, 5), 150, default, 0.7f);
                Dust.NewDust(target.Center, 0, 0, DustID.FireworkFountain_Yellow, Main.rand.NextFloat(-5, 5), Main.rand.NextFloat(-5, 5), 150, default, 0.3f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Asset<Texture2D> chainTexture = Request<Texture2D>(ChainTexturePath);
            Asset<Texture2D> chainTextureExtra = Request<Texture2D>(ChainTextureExtraPath); // This texture and related code is optional and used for a unique effect
