    ...
    public class PowderKeg : ModProjectile
    {
        ...
        private void DrawDust()
        {
            ...
            {//Circle
                ...
                Dust d = Dust.NewDustPerfect(Projectile.Center + offset, DustID.Smoke, Vector2.Zero, 200, default, 1.5f);
                Dust d2 = Dust.NewDustPerfect(Projectile.Center + offset, DustID.Flare, Vector2.Zero, 200, default, 0.4f);
            ...
            for (int i = 0; i < Main.maxProjectiles; i++)
            {
                    ...
                    other.type == ProjectileType<PowderKeg>() && other.Distance(Projectile.Center) < 250)
                {
                    for (int ir = 0; ir < 50; ir++)
                    {
                        ...
                        Dust d = Dust.NewDustPerfect(position, DustID.Smoke, null, 240, default, 1.5f);
                        Dust d2 = Dust.NewDustPerfect(position, DustID.Flare, null, 240, default, 0.7f);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                int dustIndex = Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y), 0, 0, 31, 0f + Main.rand.Next(-3, 3), 0f + Main.rand.Next(-3, 3), 100, default, 2f);
            ...
            for (int g = 0; g < 4; g++)
            {
                int goreIndex = Gore.NewGore(null, new Vector2(Projectile.position.X + Projectile.width / 2 - 7f, Projectile.position.Y + Projectile.height / 2 - 7f), default, Main.rand.Next(61, 64), 1f);
                Main.gore[goreIndex].scale = 1.2f;
                Main.gore[goreIndex].velocity.X = Main.gore[goreIndex].velocity.X + 1.5f;
                Main.gore[goreIndex].velocity.Y = Main.gore[goreIndex].velocity.Y + 1.5f;
                goreIndex = Gore.NewGore(null, new Vector2(Projectile.position.X + Projectile.width / 2 - 7f, Projectile.position.Y + Projectile.height / 2 - 7f), default, Main.rand.Next(61, 64), 1f);
                Main.gore[goreIndex].scale = 1.2f;
                Main.gore[goreIndex].velocity.X = Main.gore[goreIndex].velocity.X - 1.5f;
                Main.gore[goreIndex].velocity.Y = Main.gore[goreIndex].velocity.Y + 1.5f;
                goreIndex = Gore.NewGore(null, new Vector2(Projectile.position.X + Projectile.width / 2 - 7f, Projectile.position.Y + Projectile.height / 2 - 7f), default, Main.rand.Next(61, 64), 1f);
