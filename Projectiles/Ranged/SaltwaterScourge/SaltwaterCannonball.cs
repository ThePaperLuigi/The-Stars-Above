...
namespace StarsAbove.Projectiles.Ranged.SaltwaterScourge
{
    public class SaltwaterCannonball : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            for (int d = 0; d < 18; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.Smoke, Main.rand.NextFloat(-5, 5), Main.rand.NextFloat(-5, 5), 150, default, 0.7f);

            }
            // Large Smoke Gore spawn
            for (int g = 0; g < 4; g++)
            {
                int goreIndex = Gore.NewGore(null, new Vector2(Projectile.position.X + Projectile.width / 2 - Main.rand.NextFloat(-5, 5), Projectile.position.Y + Projectile.height / 2 - Main.rand.NextFloat(-5, 5)), default, Main.rand.Next(61, 64), 1f);
                Main.gore[goreIndex].scale = 1f;
                //Main.gore[goreIndex].velocity.X = Main.gore[goreIndex].velocity.X + 1.5f;
                //Main.gore[goreIndex].velocity.Y = Main.gore[goreIndex].velocity.Y + 1.5f;
                goreIndex = Gore.NewGore(null, new Vector2(Projectile.position.X + Projectile.width / 2 - Main.rand.NextFloat(-5, 5), Projectile.position.Y + Projectile.height / 2 - Main.rand.NextFloat(-5, 5)), default, Main.rand.Next(61, 64), 1f);
                Main.gore[goreIndex].scale = 1f;
                //Main.gore[goreIndex].velocity.X = Main.gore[goreIndex].velocity.X - 1.5f;
                //Main.gore[goreIndex].velocity.Y = Main.gore[goreIndex].velocity.Y + 1.5f;
                goreIndex = Gore.NewGore(null, new Vector2(Projectile.position.X + Projectile.width / 2 - Main.rand.NextFloat(-5, 5), Projectile.position.Y + Projectile.height / 2 - Main.rand.NextFloat(-5, 5)), default, Main.rand.Next(61, 64), 1f);
