...
{
    public class CoruscantSaberOut : ModProjectile
	{
        ...
        public override void AI() {
			...
			if (Projectile.ai[1] >= 20)
            {
				...
				for (int i = 0; (float)i < dustAmount; i++)
				{
					...
					Main.dust[dust].velocity = Projectile.velocity * 0f + spinningpoint5.SafeNormalize(Vector2.UnitY) * 32f;
				}
				Projectile.ai[1] = 0;
            }
			...
			if(Projectile.ai[0] == 0)
			{
				...
				Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ModContent.ProjectileType<CoruscantSaberAnimation>(), 0, 0f, Main.myPlayer, 0, 1);
				Main.LocalPlayer.GetModPlayer<StarsAbovePlayer>().screenShakeTimerGlobal = -80;

				for (int i = 0; i < 45; i++)
				{
					Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y), 0, 0, DustID.FireworkFountain_Yellow, 0f + Main.rand.Next(-18, 18), 0f + Main.rand.Next(-18, 18), 150, default(Color), 1.7f);
				...
				for (int i = 0; i < 43; i++)
				{
					Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y), 0, 0, DustID.GemTopaz, 0f + Main.rand.Next(-18, 18), 0f + Main.rand.Next(-18, 18), 150, default(Color), 2.7f);
