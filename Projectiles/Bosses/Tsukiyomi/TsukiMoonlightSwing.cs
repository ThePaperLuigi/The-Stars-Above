...
{
    public class TsukiMoonlightSwing : ModProjectile
	{
		...
		public override void SetDefaults() {
			Projectile.width = 108;               //The width of projectile hitbox
		...
		public override void AI()
		{
			...
			if (!NPC.AnyNPCs(ModContent.NPCType<TsukiyomiBoss>()) && !NPC.AnyNPCs(ModContent.NPCType<NalhaunBossPhase2>()))//Temp
			{
			...
			deg = MathHelper.Lerp(0, 359, EaseHelper.InOutQuad(rotationProgress)) + Projectile.ai[1];
			...
			for (int i = 0; i < Main.maxNPCs; i++)
			{
				...
				if (other.active && (other.type == ModContent.NPCType<TsukiyomiBoss>() || other.type == ModContent.NPCType<NalhaunBossPhase2>()))
				{
					...
					Projectile.rotation = Vector2.Normalize(other.Center - Projectile.Center).ToRotation() + MathHelper.ToRadians(-45f);
					return;
				}
			}
