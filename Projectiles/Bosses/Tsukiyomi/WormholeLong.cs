...
namespace StarsAbove.Projectiles.Bosses.Tsukiyomi
{
    public class WormholeLong : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            for (int d = 0; d < 26; d++)
            {
                Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y), 0, 0, 20, 0f + Main.rand.Next(-16, 16), 0f + Main.rand.Next(-16, 16), 0, default, 1f);
            }
            for (int d = 0; d < 22; d++)
            {
                Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y), 0, 0, 221, 0f + Main.rand.Next(-16, 16), 0f + Main.rand.Next(-16, 16), 0, default, 1f);
            }
