...
{
	public class TsukiStygianMemento2 : ModProjectile
	{
		...
		public override void AI() {
			...
			if (Projectile.ai[1] >= 5)
			{
				if (foundTarget)
				{
					...
					Projectile.NewProjectile(Projectile.GetSource_FromThis(), position.X, position.Y - 600, 0, 5, type, Projectile.damage, 0f, Main.myPlayer);
					Projectile.NewProjectile(Projectile.GetSource_FromThis(), position.X, position.Y + 600, 0, -5, type, Projectile.damage, 0f, Main.myPlayer);
