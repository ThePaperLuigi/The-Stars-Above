...
namespace StarsAbove.Projectiles.Bosses.Tsukiyomi
{
    public class TsukiOutbreakRound : ModProjectile
	{
		...
		public override void OnKill(int timeLeft)
		{
			for (int d = 0; d < 6; d++)
			{
				Dust.NewDust(new Vector2(Projectile.Center.X, Projectile.Center.Y), 0, 0, DustID.LifeDrain, 0f + Main.rand.Next(-3, 3), 0f + Main.rand.Next(-3, 3), 150, default(Color), 0.7f);
			}
