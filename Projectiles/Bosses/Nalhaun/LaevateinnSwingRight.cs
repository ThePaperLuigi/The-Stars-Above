			...
			if(Projectile.ai[0] == 0)
            {
				...
				for (int ir = 0; ir < Main.maxNPCs; ir++)
				{
					...
					if (npc.active && npc.type == ModContent.NPCType<NalhaunBoss>())
					{
						...
						for (int i = 0; i < 60; i++)
						{
							Dust.NewDust(new Vector2(npc.Center.X + 60, npc.Center.Y), 0, 0, DustID.FireworkFountain_Red, 0f + Main.rand.Next(0, 38), 0f + Main.rand.Next(-18, 18), 150, default(Color), 1.7f);

						}
						for (int i = 0; i < 50; i++)
						{
							Dust.NewDust(new Vector2(npc.Center.X + 60, npc.Center.Y), 0, 0, DustID.Flare, 0f + Main.rand.Next(0, 28), 0f + Main.rand.Next(-18, 18), 150, default(Color), 3.7f);

						}
						for (int i = 0; i < 50; i++)
						{
							Dust.NewDust(new Vector2(npc.Center.X + 60, npc.Center.Y), 0, 0, DustID.LifeDrain, 0f + Main.rand.Next(0, 48), 0f + Main.rand.Next(-18, 18), 150, default(Color), 2.7f);
