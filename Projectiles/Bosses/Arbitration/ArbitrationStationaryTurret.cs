...
{
    public class ArbitrationStationaryTurret : ModProjectile
	{
		...
		private void SearchForTargets(out bool foundTarget, out Vector2 targetCenter)
		{
			...
			if (!foundTarget)
			{
				...
				for (int i = 0; i < Main.maxNPCs; i++)
				{
					...
					if (npc.active && npc.type == ModContent.NPCType<ArbitrationBoss>() || npc.type == ModContent.NPCType<NalhaunBossPhase2>())
					{
						foundTarget = true;
