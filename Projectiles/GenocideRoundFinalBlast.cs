...
namespace StarsAbove.Projectiles
{
	public class GenocideRoundFinalBlast : ModProjectile
	{
		...
		public override void AI() {
			...
			for (int d = 0; d < 26; d++)
			{
				Dust.NewDust(Projectile.Center, 0, 0, 134, 0f + Main.rand.Next(-8, 8), 0f + Main.rand.Next(-8, 8), 150, default(Color), 1.5f);
			}
			for (int d = 0; d < 20; d++)
			{
				Dust.NewDust(Projectile.Center,0, 0, 7, 0f + Main.rand.Next(-5, 5), 0f + Main.rand.Next(-5, 5), 150, default(Color), 1.5f);
			}
			for (int d = 0; d < 20; d++)
			{
				Dust.NewDust(Projectile.Center, 0, 0, 269, 0f + Main.rand.Next(-5, 5), 0f + Main.rand.Next(-5, 5), 150, default(Color), 1.5f);
			}
			for (int d = 0; d < 30; d++)
			{
				Dust.NewDust(Projectile.Center, 0, 0, 78, 0f + Main.rand.Next(-5, 5), 0f + Main.rand.Next(-5, 5), 150, default(Color), 1.5f);
			}
			// Smoke Dust spawn
			for (int i = 0; i < 20; i++)
			{
