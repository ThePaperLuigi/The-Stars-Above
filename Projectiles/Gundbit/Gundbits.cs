...
namespace StarsAbove.Projectiles.Gundbit
{
    ...
    public class Gundbits : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (Projectile.ai[0] > 90)
			{
				...
				if (foundTarget && distanceFromTarget < 300)
				{
					...
					for (int d = 0; d < 5; d++)
					{
						Dust.NewDust(Projectile.Center, 0, 0, DustID.Electric, 0f + Main.rand.Next(-3, 3), 0f + Main.rand.Next(-3, 3), 150, default(Color), 0.5f);
					}
			...
			if (owner.HasBuff(BuffType<GundbitBeamAttack>()))
			{
				if(Projectile.alpha <= 200)
                {
					for (int d = 0; d < 5; d++)
					{
						int dustIndex = Dust.NewDust(Projectile.Center, 0, 0, DustID.GemSapphire, 0f + Main.rand.Next(-1, 1), 0f + Main.rand.Next(-1, 1), 0, default(Color), 2f);
        ...
        private bool CheckActive(Player owner)
		{
			...
			{
				owner.ClearBuff(ModContent.BuffType<AerialDefensiveArray>());

				return false;
			}

			if (owner.HasBuff(ModContent.BuffType<AerialDefensiveArray>()))
			{
