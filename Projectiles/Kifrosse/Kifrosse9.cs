...
namespace StarsAbove.Projectiles.Kifrosse
{
    public class Kifrosse9 : ModProjectile
	{
		...
		public override void AI()
		{
			...
			{
				int index = Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.position.X, player.position.Y, 0, 0, ProjectileType<Projectiles.Kifrosse.BlizzardFoxfire1>(), Projectile.damage, 4, player.whoAmI, 0f);

				Main.projectile[index].originalDamage = Projectile.damage;

			}
			if (player.ownedProjectileCounts[ProjectileType<Projectiles.Kifrosse.BlizzardFoxfire2>()] < 1)
			{
				int index = Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.position.X, player.position.Y, 0, 0, ProjectileType<Projectiles.Kifrosse.BlizzardFoxfire2>(), Projectile.damage, 4, player.whoAmI, 0f);

				Main.projectile[index].originalDamage = Projectile.damage;

			}
			if (player.ownedProjectileCounts[ProjectileType<Projectiles.Kifrosse.BlizzardFoxfire3>()] < 1)
			{
				int index = Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.position.X, player.position.Y, 0, 0, ProjectileType<Projectiles.Kifrosse.BlizzardFoxfire3>(), Projectile.damage, 4, player.whoAmI, 0f);
