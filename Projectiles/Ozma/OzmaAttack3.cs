...
namespace StarsAbove.Projectiles.Ozma
{
    public class OzmaAttack3 : ModProjectile
	{
        ...
        public override void AI()
        {
			...
			Dust.NewDust(Projectile.Center, 0, 0, DustID.PlatinumCoin, 0f , 0f + Main.rand.Next(-1, 1), 150, default(Color), 1f);
			for (int d = 0; d < 10; d++)
			{
		...
		public override void Kill(int timeLeft)
		{
			//for (int d = 0; d < 8; d++)
			{
				Dust.NewDust(Projectile.Center, 0, 0, ProjectileID.DiamondBolt, Main.rand.NextFloat(-7, 7), Main.rand.NextFloat(-7, 7), 150, default(Color), 0.9f);
