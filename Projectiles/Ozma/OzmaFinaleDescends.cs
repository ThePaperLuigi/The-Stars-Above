    ...
    public class OzmaFinaleDescends : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (Projectile.ai[0] > 30)
            {
				...
				for (int d = 0; d < 16; d++)
				{
					Dust.NewDust(Projectile.Center, 0, 0, DustID.Blood, 0f + Main.rand.Next(-16, 16), 0f + Main.rand.Next(-16, 16), 150, default(Color), 0.5f);
				}
				for (int d = 0; d < 10; d++)
				{
					Dust.NewDust(Projectile.Center, 0, 0, DustID.LifeDrain, 0f + Main.rand.Next(-13, 13), 0f + Main.rand.Next(-13, 13), 150, default(Color), 0.5f);
				}
				for (int d = 0; d < 10; d++)
				{
					Dust.NewDust(Projectile.Center, 0, 0, DustID.FireworkFountain_Red, 0f + Main.rand.Next(-13, 13), 0f + Main.rand.Next(-13, 13), 150, default(Color), 0.5f);
				}
