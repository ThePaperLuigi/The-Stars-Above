    ...
    public class KarnaSpearThrow : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft == 56)
            {
                ...
                {
                    Vector2 spinningpoint5 = Vector2.UnitX * 0f;
                    spinningpoint5 += -Vector2.UnitY.RotatedBy(i * ((float)Math.PI * 2f / dustAmount)) * new Vector2(1f, 8f);
                    spinningpoint5 = spinningpoint5.RotatedBy(Projectile.velocity.ToRotation());
                    int dust = Dust.NewDust(Projectile.Center, 0, 0, DustID.LifeDrain);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (Projectile.penetrate != 0)
            {
                for (int d = 0; d < 23; d++)//Visual effects
                {
                    ...
                    perturbedSpeed = perturbedSpeed * scale;
                    int dustIndex = Dust.NewDust(Projectile.Center, 0, 0, DustID.LifeDrain, perturbedSpeed.X, perturbedSpeed.Y, 150, default, 0.6f);
                    Main.dust[dustIndex].noGravity = true;
