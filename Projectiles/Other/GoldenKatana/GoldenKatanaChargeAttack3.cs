...
namespace StarsAbove.Projectiles.Other.GoldenKatana
{
    ...
    public class GoldenKatanaChargeAttack3 : ModProjectile
    {
        ...
        public override void SetDefaults()
        {
            ...
            Projectile.DamageType = GetInstance<AuricDamageClass>();
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int d = 0; d < 12; d++)
            {
                Dust.NewDust(target.Center, 0, 0, DustID.PinkCrystalShard, 0f + Main.rand.Next(-4, 4), 0f + Main.rand.Next(-12, 12), 150, default, 0.8f);
            }
            for (int d = 0; d < 12; d++)
            {
                Dust.NewDust(target.Center, 0, 0, DustID.PinkFairy, 0f + Main.rand.Next(-22, 22), 0f + Main.rand.Next(-12, 12), 150, default, 0.8f);
            }
            for (int d = 0; d < 32; d++)
            {
                Dust.NewDust(target.Center, 0, 0, DustID.FireworkFountain_Pink, 0f + Main.rand.Next(-36, 36), 0f + Main.rand.Next(-4, 4), 150, default, 1.1f);
            }
            for (int d = 0; d < 5; d++)
            {
                Dust.NewDust(target.Center, 0, 0, 7, 0f + Main.rand.Next(-7, 7), 0f + Main.rand.Next(-7, 7), 150, default, 1.5f);
            }
            for (int d = 0; d < 6; d++)
            {
                Dust.NewDust(target.Center, 0, 0, 269, 0f + Main.rand.Next(-6, 6), 0f + Main.rand.Next(-6, 6), 150, default, 1.5f);
            }
            for (int d = 0; d < 7; d++)
            {
                Dust.NewDust(target.Center, 0, 0, 78, 0f + Main.rand.Next(-4, 4), 0f + Main.rand.Next(-4, 4), 150, default, 1.5f);
            }
            // Smoke Dust spawn
            for (int i = 0; i < 10; i++)
            {
