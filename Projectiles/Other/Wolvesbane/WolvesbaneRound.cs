...
{
    public class WolvesbaneRound : ModProjectile
    {
        public override void SetStaticDefaults()
        {
            // DisplayName.SetDefault("Outbreak Perfected Round");     //The English name of the projectile
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            projOwner.GetModPlayer<WeaponPlayer>().wolvesbaneGauge += 5f;

            for (int d = 0; d < 8; d++)
            {
                Dust.NewDust(target.Center, 0, 0, DustID.LifeDrain, Main.rand.NextFloat(-5, 5), Main.rand.NextFloat(-5, 5), 150, default, 0.4f);
            ...
            for (int d = 0; d < 23; d++)//Visual effects
            {
                ...
                float scale = 0f + Main.rand.NextFloat() * 0.6f;
                perturbedSpeed = perturbedSpeed * scale;
                int dustIndex = Dust.NewDust(Projectile.Center, 0, 0, DustID.LifeDrain, perturbedSpeed.X, perturbedSpeed.Y, 150, default, 0.9f);
