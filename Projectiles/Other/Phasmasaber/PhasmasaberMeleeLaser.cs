...
{
    public class PhasmasaberMeleeLaser : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.localAI[0] > 9f)
            {
                for (int i = 0; i < 4; i++)
                {
                    ...
                    int dust = Dust.NewDust(ProjectilePosition, 1, 1, DustID.ShadowbeamStaff, 0f + Main.rand.Next(-4, 4), 0f + Main.rand.Next(-4, 4), 0, default, 2f);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int d = 0; d < 7; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.UnusedWhiteBluePurple, 0f + Main.rand.Next(-13, 13), 0f + Main.rand.Next(-3, 3), 150, default, 1.5f);
            }
            for (int d = 0; d < 46; d++)
            {
                Dust.NewDust(Projectile.Center, 0, 0, DustID.ShadowbeamStaff, 0f + Main.rand.Next(-26, 26), 0f + Main.rand.Next(-26, 26), 150, default, 1.5f);
            }
