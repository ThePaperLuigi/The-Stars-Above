...
namespace StarsAbove.Projectiles.RedMage
{
    public class Verstone : ModProjectile
	{
        ...
        public override void AI()
        {
			for (int d = 0; d < 2; d++)
			{
				Dust.NewDust(Projectile.Center, 0, 0, DustID.Dirt, Main.rand.NextFloat(-2, 2), Main.rand.NextFloat(-2, 2), 150, default(Color), 0.4f); 
				Dust.NewDust(Projectile.Center, 0, 0, DustID.GemAmber, Main.rand.NextFloat(-2, 2), Main.rand.NextFloat(-2, 2), 150, default(Color), 0.7f);
		...
		public override void Kill(int timeLeft)
		{
			...
			for (int d = 0; d < 16; d++)
			{
				Dust.NewDust(Projectile.Center, 0, 0, DustID.Firework_Yellow, 0f + Main.rand.Next(-2, 2), 0f + Main.rand.Next(-2, 2), 150, default(Color), 1.5f);
			}
			for (int d = 0; d < 10; d++)
			{
			...
			for (int d = 0; d < 10; d++)
			{
				Dust.NewDust(Projectile.Center, 0, 0, DustID.AmberBolt, 0f + Main.rand.Next(-3, 3), 0f + Main.rand.Next(-3, 3), 150, default(Color), 1.5f);
			}
			// Smoke Dust spawn
			for (int i = 0; i < 20; i++)
			{
