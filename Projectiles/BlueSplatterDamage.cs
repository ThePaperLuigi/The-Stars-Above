...
namespace StarsAbove.Projectiles
{
    public class BlueSplatterDamage : ModProjectile
	{
		...
		public override void AI() {
			...
			for (int d = 0; d < 130; d++)
			{
				Dust.NewDust(Projectile.Center, 0, 0, 132, 0f + Main.rand.Next(-10, 10), 0f + Main.rand.Next(-100, 100), 150, default(Color), 1.5f);
			}
			for (int d = 0; d < 144; d++)
			{
				Dust.NewDust(Projectile.Center, 0, 0, 132, 0f + Main.rand.Next(-15, 15), 0f + Main.rand.Next(-150, 150), 150, default(Color), 1.5f);
			}
			for (int d = 0; d < 126; d++)
			{
				Dust.NewDust(Projectile.Center, 0, 0, 132, 0f + Main.rand.Next(-16, 16), 0f + Main.rand.Next(-160, 160), 150, default(Color), 1.5f);
			}
			for (int d = 0; d < 30; d++)
			{
				Dust.NewDust(Projectile.Center, 0, 0, 132, 0f + Main.rand.Next(-13, 13), 0f + Main.rand.Next(-130, 130), 150, default(Color), 1.5f);
			}
			for (int d = 0; d < 40; d++)
			{
				Dust.NewDust(Projectile.Center, 0, 0, 132, 0f + Main.rand.Next(-13, 13), 0f + Main.rand.Next(-130, 130), 150, default(Color), 1.5f);
