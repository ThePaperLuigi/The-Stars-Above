...
{
    public class ChemtankMarker4 : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (Projectile.ai[0] <= 0)
			{
				...
				if (p.HasBuff(BuffType<Buffs.Chemtank.Chemtank4>()))
				{
					p.ClearBuff(BuffType<Buffs.Chemtank.Chemtank4>());
					Projectile.ai[0] = 480;
					int type = ProjectileType<Projectiles.Chemtank.ChemtankRound>();
					...
					Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), position.X, position.Y, muzzleOffset.X, muzzleOffset.Y, ProjectileType<Projectiles.Chemtank.ChemtankExplosion>(), 40, 2, player.whoAmI);
					...
					for (int d = 0; d < 21; d++)
					{
						...
						perturbedSpeed = perturbedSpeed * scale;
						int dustIndex = Dust.NewDust(position, 0, 0, DustID.FireworkFountain_Green, perturbedSpeed.X, perturbedSpeed.Y, 150, default(Color), 1f);
