...
namespace StarsAbove.Buffs.Pets
{
    public class MajimaPetBuff : ModBuff
    {
        ...
        {
            player.GetModPlayer<WeaponPlayer>().MajimaPet = true;
            player.buffTime[buffIndex] = 18000;
            bool petProjectileNotSpawned = player.ownedProjectileCounts[ProjectileType<Projectiles.Pets.MajimaPet>()] <= 0;
            if (petProjectileNotSpawned && player.whoAmI == Main.myPlayer)
            {
                Projectile.NewProjectile(player.GetSource_Buff(buffIndex), player.position.X + player.width / 2, player.position.Y + player.height / 2, 0f, 0f, ProjectileType<Projectiles.Pets.MajimaPet>(), 0, 0f, player.whoAmI, 0f, 0f);
            }
